...
def run(playwright: Playwright, gist_id: str, filename) -> None:
    ...
    page.goto(
        "https://vpn1.fnvpn1.top/#/stage/dashboard",
        wait_until="networkidle",  # 页面网络空闲时认为加载完成
        timeout=30000
    )
    print("登录成功，已进入用户后台。")
    ...
    # --- 3. 获取并保存订阅链接 ---
    print("正在获取订阅链接...")
    # page.get_by_text("我的订阅").first.click()
    page.locator(
        "xpath=((//ul[contains(@class,'link')])[2]//li)[last()]"
    ).click()

    # 等待“复制”按钮出现并点击
    # page.get_by_text("复制").first.click()
